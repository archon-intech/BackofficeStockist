...
<template>
  ...
  <AuthenticatedLayout>
    <div class="mb-5">
      <h5 class="text-h5 font-weight-bold">Store</h5>
    ...
    <v-card class="pa-4">
      ...
      <v-data-table-server :items="data.data" :items-length="data.total" :headers="headers" :search="search"
        ...
        <template #[`item.action`]="{ item }">
          <Link :href="`/store/${item.id}/edit`" as="button">
          <v-icon color="warning" icon="mdi-pencil" size="small" />
          </Link>
          <Link :href="`/store/${item.id}`" as="button">
...
export default {
  ...
  methods: {
    ...
    submitPassword() {
      this.isLoading = true;
      this.$inertia.post(`/store/activate/${this.userId}`, { password: this.password }, {
        onSuccess: () => {
    ...
    submitDelete() {
      this.isLoading = true
      this.$inertia.delete(`/store/${this.deleteId}`, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
          this.isLoading = false
