...
<template>
  ...
  <AuthenticatedLayout>
    <div class="mb-5">
      <h5 class="text-h5 font-weight-bold">Promo</h5>
    ...
    <v-card class="pa-4">
      ...
      <v-data-table-server :items="data.data" :items-length="data.total" :headers="headers" :search="search"
        ...
        <template #[`item.status`]="{ item }">
          <div>
            <v-btn v-if="item.status == 1" @click="showPasswordDialog(item.id)" color="success">Aktif</v-btn>
            <v-btn v-else color="error" @click="showPasswordDialog(item.id)">Inactive</v-btn>
          </div>
        </template>
        <template #[`item.action`]="{ item }">
          <Link :href="`/promo/${item.id}/edit`" as="button">
...
export default {
  ...
  methods: {
    ...
    submitPassword() {
      this.isLoading = true;
      this.$inertia.post(`/promo/activate/${this.userId}`, { password: this.password }, {
        onSuccess: () => {
    ...
    submitDelete() {
      this.isLoading = true
      this.$inertia.delete(`/promo/${this.deleteId}`, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
          this.isLoading = false
