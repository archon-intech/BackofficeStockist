...
<template>
  ...
  <Head title="Manufacture" />
  <AuthenticatedLayout>
    <div class="mb-5">
      <h5 class="text-h5 font-weight-bold">Manufacture</h5>
    ...
    <v-card class="pa-4">
      <div class="d-flex flex-wrap align-center">
        ...
        <v-spacer />
        <!-- <Link href="/manufacture/create" as="div"> -->
      ...
      <v-data-table-server :items="data.data" :items-length="data.total" :headers="headers" :search="search"
        ...
        <template #[`item.user.is_blocked`]="{ item }">
          <div>
            <v-btn v-if="item.user.is_blocked === 0" color="success">Active</v-btn>
        ...
        <template #[`item.action`]="{ item }">
          <Link :href="`/manufacture/${item.id}/edit`" as="button">
...
export default {
  ...
  methods: {
    loadItems({ page, itemsPerPage, sortBy, search }) {
      ...
      this.$inertia.get('/manufacture', params, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
    ...
    submitDelete() {
      this.isLoading = true
      this.$inertia.delete(`/manufacture/${this.deleteId}`, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
